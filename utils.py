...
def format_record(record, fld_attrs):
    ...
    for i,x in enumerate(zip(vals, field_types)):
        ...
        if tmp is None:
            ...
            if tmp is None:
                print("TIME", x[0], record)
        elif x[1] == 'DATE':
            tmp = convert_to_date(x[0])
            if tmp is None:
                print("DATE", x[0], record)
        elif x[1] == 'INTEGER':
            tmp = convert_to_int(x[0])
            if tmp is None:
                print("INTEGER", x[0], record)
        elif x[1] == 'FLOAT':
            tmp = convert_to_float(x[0])
            if tmp is None:
                print("FLOAT", x[0], record)
        vals[i] = tmp
...
def convert_to_float(val):
    ...
    extraniouse spaces that often appear in this field
